...
try:
    create_file_response = requests.post(
        f"{namenode_url}/files", json={"filename": filename, "size": file_size}
    )
    ...
    for block in block_assignments:
        ...
        for datanode in datanodes:
            ...
            try:
                ...
                print(
                    f"Block {block_number} uploaded to {datanode['host']}:{datanode['port']}"
                )
            except requests.exceptions.RequestException as e:
                print(
                    f"Failed to upload block {block_number} to {datanode['host']}:{datanode['port']}: {e}"
                )
