...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(
            "Usage: spark-submit spark_tweet_user_retweets.py <language> <input_path>"
        )
        sys.exit(1)
    ...
    filtered_df = df.filter(col("lang") == language).select(
        "user.screen_name", "retweet_count"
    )

    # Aggregate retweets per user
    retweet_counts = filtered_df.groupBy("screen_name").agg(
